	...
	const handleSendMessage = async (content: string) => {
		...
		try {
			...
			const response = await fetch(
				'http://localhost:3000/a9f3105f-7b14-09bd-919f-a1482637a374/message',
				{
					method: 'POST',
					headers: {
		...
		<Dialog.Root>
			<Dialog.Trigger asChild>
				<button className='w-full flex items-center gap-3 px-4 py-3 bg-pink-950/20 hover:bg-pink-950/30 rounded-lg transition-all duration-200'>
					...
					<span className='font-semibold text-pink-300 text-base'>
						Launch Streamer
					</span>
				</button>
			</Dialog.Trigger>
			<Dialog.Portal>
				<Dialog.Content className='fixed inset-4 sm:inset-auto sm:left-[50%] sm:top-[50%] sm:max-w-[640px] sm:w-full sm:translate-x-[-50%] sm:translate-y-[-50%] sm:h-[80vh] bg-gray-900 rounded-2xl shadow-xl flex flex-col z-[9999] animate-in fade-in zoom-in-95 duration-200'>
